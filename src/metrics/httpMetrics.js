...
function requestTracker(req, res, next) {
  ...
  res.on("finish", () => {
    ...
    httpErrors[statusKey] = (httpErrors[statusKey] || 0) + 1;
  });
  next();
}
function httpMetrics() {
  ...
  metrics.push(
    createMetric("http_requests_total", total, "1", "sum", "asInt", {}),
  );
  ...
  if (allLatencies.length > 0) {
    ...
    metrics.push(
      createMetric("http_latency_ms", avg, "ms", "gauge", "asDouble", {}),
    );
  }

  const serverErrors = Object.entries(httpErrors)
    .filter(([code]) => code.startsWith("5"))
    .reduce((a, [, b]) => a + b, 0);

  metrics.push(
    createMetric("http_errors_5xx", serverErrors, "1", "sum", "asInt", {}),
  );

  // Per status code breakdown
  Object.entries(httpErrors).forEach(([code, count]) => {
    metrics.push(
      createMetric("http_errors_by_status", count, "1", "sum", "asInt", {
        status: code,
      }),
    );
  });
